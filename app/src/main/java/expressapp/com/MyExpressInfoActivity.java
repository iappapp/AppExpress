...
public class MyExpressInfoActivity extends Activity implements OnClickListener {

	private ListView lv_express_info;
	private String number;
	private String code;
	private String name;
	private Button mybtnBack;// 返回按钮
	private Button mybtnSaveOrBack;// 保存按钮
	private DbUtils db;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.my_expresss_info_activity);
		infoList = getIntent().getExtras().getParcelableArrayList("infos");
		lv_express_info = (ListView) findViewById(R.id.my_lv_express_info_list);
		lv_express_info.setAdapter(new ExpressInfoAdapter(this, infoList));
		number = getIntent().getStringExtra("number");
		code = getIntent().getStringExtra("code");
		name = getIntent().getStringExtra("name");
		mybtnSaveOrBack = (Button) findViewById(R.id.my_btn_info_save_or_delete);
		mybtnSaveOrBack.setOnClickListener(this);
		mybtnBack = (Button) findViewById(R.id.my_btn_info_back);
	...
	public void onClick(View v) {
		// TODO Auto-generated method stub
		switch (v.getId()) {
		case R.id.my_btn_info_back:
			finish();
			break;
		case R.id.my_btn_info_save_or_delete:
			if (mybtnSaveOrBack.getText().toString().equals("保存")) {
						...
						.setView(editText).setPositiveButton("确定", new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								...
								try {
									...
									Toast.makeText(MyExpressInfoActivity.this, "保存成功", Toast.LENGTH_SHORT).show();
									mybtnSaveOrBack.setText("删除");
								} catch (DbException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
									Toast.makeText(MyExpressInfoActivity.this, "啊哦，保存失败了呢", Toast.LENGTH_SHORT).show();
								}
							}
			...
			} else if (mybtnSaveOrBack.getText().toString().equals("删除")) {
				try {
					db.deleteById(ExpressHistory.class, isExist.getId());
					Toast.makeText(MyExpressInfoActivity.this, "删除成功", Toast.LENGTH_SHORT).show();
					finish();
				} catch (DbException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
